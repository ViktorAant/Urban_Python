...
kb = ReplyKeyboardMarkup(resize_keyboard=True).add(calculate, buy, reg, information)
...
async def set_username(message, state):
    if ff.is_included(message.text):
        await message.reply(f'Пользователь {message.text} существует, введите другое имя', reply=False)
        return
...
async def set_age(message, state):
    ...
    async def set_age(message, state):
        ...
        data = await state.get_data()
        ff.add_user(data['username'], data['email'], data['age'])
        await message.reply('Регистрация прошла успешно', reply=False)
        await state.finish()
